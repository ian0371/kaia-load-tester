...
func SendRandomTx(cli *ethclient.Client, from *account.Account) error {
	...
	// - tx1: TPSL order $3 BUY slTrigger=$2 slLimit=$1 (20%)
	// - tx2: Limit order $3 SELL (20%)
	// - tx3: limit order $2 BUY (20%)
	// - tx4: limit order $2 SELL (20%)
	// - tx5: limit order $1 BUY (20%)
	...
	case 0:
		...
		slTrigger = scaleUp(2)
		slLimit = scaleUp(1)
		tx, err = from.GenNewOrderTxWithTpsl(baseToken, quoteToken, side, price, quantity, orderType, tpLimit, slTrigger, slLimit)
		if err != nil {
